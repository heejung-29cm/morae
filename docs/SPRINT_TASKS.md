...
| Sprint 3 | 완료 | 수동 생성, 무재시도, 아티클 전용 결과 UI, 최신 결과 복원 |
...
| S3-06 | S | 첫 브리핑 최우선 할 일 질문 | S1-02, S3-05 | 구현 후 S3.2-01에서 제거했습니다. |
...
## 5.2 Sprint 3.2 — 개별 아티클 품질과 화면 단순화

목표: 뉴스레터 호 대신 신선한 개별 글을 추천하고 오늘의 아티클 섹션을
아티클 전용으로 유지합니다.

| ID | 크기 | 태스크 | 선행 | 완료·검토 기준 |
| --- | --- | --- | --- | --- |
| S3.2-01 | S | 아티클 전용 UI | S3-05 | 아티클 섹션에서 할 일 요약과 첫 실행 질문을 제거하고 버튼 즉시 한 번 생성합니다. |
| S3.2-02 | S | 개별 글 기본 소스 전환 | S3.1-01 | GeekNews와 Korean FE Article만 기본 활성화하고 뉴스레터 호 중심 소스는 v3 seed에서 비활성화합니다. |
| S3.2-03 | S | 신선도 eligibility | S2-07 | 게시일이 있고 최근 30일 이내인 후보만 점수 계산에 포함합니다. |
| S3.2-04 | XS | 제목 정제·ADR·회귀 검증 | S3.2-02, S3.2-03 | 반복 소스 접두어 제거, ADR-0014와 자동화 테스트가 코드와 일치합니다. |

## 5.3 Sprint 3.3 — 개인 주제 우선순위

목표: 최신 후보 안에서 사용자가 선호하는 AI·프론트엔드와 협업 글을
인프라·데이터 및 기타 주제보다 우선 추천합니다.

| ID | 크기 | 태스크 | 선행 | 완료·검토 기준 |
| --- | --- | --- | --- | --- |
| S3.3-01 | S | 제목 기반 주제 분류 | S3.2-03 | `ArticleTopicClassifier`가 한국어·영어 제목을 AI·FE, 협업, 인프라·데이터, 기타로 결정론적으로 분류합니다. |
| S3.3-02 | XS | 주제 점수 적용 | S3.3-01 | 120, 45, 20, 0 점수를 기존 선정식에 추가하고 최근 추천 감점은 유지합니다. |
| S3.3-03 | XS | 회귀 테스트·ADR | S3.3-02 | 오래된 상위 주제와 최신 하위 주제, 협업·인프라 순서를 고정 Clock으로 검증하고 ADR-0015와 일치합니다. |
...
| 항목 | 상태 | 확인 결과 |
| --- | --- | --- |
| 큐레이션 피드 URL·이용 정책 | 완료 (2026-07-30) | GeekNews와 Korean FE Article의 공식 HTTPS 피드와 XML을 확인했습니다. 제목·링크·출처·게시일만 사용하는 정책을 유지합니다. |
