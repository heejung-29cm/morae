...
- pending Todo에는 6-dot drag affordance를 표시하고 insertion indicator에
  accent를 사용합니다.
- 내부 순서 변경은 `MenuBarExtra(.window)`에서 시스템 drop destination에
  의존하지 않습니다. 3pt threshold `DragGesture`, named coordinate space와
  row frame으로 pointer 위치를 계산하고 mouse-up에서 한 번 확정합니다.
- drag 중인 Todo는 반투명 preview로 포인터를 따라가고 원본 row는 흐리게
  표시합니다.
...
- 메뉴 바 전용 window의 drop destination 활성화 여부와 무관하게 내부
  재정렬 피드백을 일관되게 제어할 수 있습니다.
...
- 로컬 pointer drag는 외부 앱으로의 drag-and-drop을 지원하지 않으며,
  row frame과 coordinate space의 정합성을 회귀 검증해야 합니다.
