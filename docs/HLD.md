...
재시작 후 같은 날짜의 최신 결과를 복원합니다. 오늘의 아티클 섹션에는
할 일 요약을 중복 표시하지 않고 추천 링크만 표시합니다. 기본 소스는
...
- 후보를 주제 우선순위, 게시 시점, 관심 분야, 기존 추천 여부, 공식 출처
  여부로 점수화합니다.
- 유효 후보가 없으면 아티클 영역에 "추천할 새 글이 없음" 상태를
  표시하고 저장합니다. 독립된 할 일 섹션은 그대로 유지합니다.
