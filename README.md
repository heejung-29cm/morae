...
> 문서 상태: MVP 사양 확정 / Sprint 0~7 구현 완료
> 최초 작성: 2026-07-24  
> 최종 수정: 2026-07-31
...
2026-08-03 기준으로 Sprint 0부터 Sprint 8까지 구현됐습니다.
...
- Mac의 현재 Calendar와 time zone에서 자정이 지나거나 시스템
  시계·time zone이 변경되면 앱 재시작 없이 오늘/어제 목록과 날짜별
  관찰 대상을 새 날짜로 전환합니다.
...
- GeekNews와 Korean FE Article의 개별 글 중심 큐레이션 피드를 기본으로
  사용합니다. 제한된 HTTPS client와 RSS/Atom
  메타데이터 parser가 제목·링크·출처·게시일만 읽습니다.
- 게시일이 있는 최근 30일 후보만 대상으로 canonical URL 중복 제거,
  AI·프론트엔드 ≫ 협업 > 인프라·데이터 순의 주제 우선순위와
  최신성·관심사·공식 출처·읽음·최근 추천 이력을 반영한 결정론적 선정,
  로컬 Article 저장소가 구현됐습니다.
- `아티클 추천받기`를 누른 경우에만 최대 4개 피드를 각각 한 번 조회해
  오늘의 아티클 섹션에 추천 아티클 한 개만 표시합니다. 부분·전체 실패,
  중복 클릭 차단과 재시작 후 최신 결과 복원을 지원합니다.
- 피드 자동 재시도나 재시도 버튼은 없으며 앱 실행과 메뉴 열기만으로
  네트워크 요청을 시작하지 않습니다.
- 첫 실행에는 현재 Space에 5단계 온보딩이 열립니다. 관심사,
  Codex·Claude 자동 연결과 알림, 선택적 Jira 연결을 한 흐름에서
  설정하며 일반 설정에서 다시 볼 수 있습니다.
- 아티클의 `관심 없음`, `이 주제 더 보기`, `나중에 읽기` 피드백을
  로컬에 저장합니다. 제외 URL과 선호 주제·출처는 다음 수동 추천의
  후보 필터와 제한된 가중치에 반영되고 외부로 전송되지 않습니다.

### 첫 실행 테스트 프로필

기존 데이터와 설정을 유지한 채 처음 설치한 화면을 확인하려면 일반
Morae를 종료한 뒤 다음처럼 별도 인스턴스를 실행합니다.

```bash
open -na /Applications/Morae.app --args --fresh-test-profile
```

테스트 프로필은 프로세스별 임시 DB와 별도 UserDefaults를 사용합니다.
Jira 토큰은 메모리에만 두고, Claude·Codex 자동 설정은 격리된 가상 홈에
적용하며, 로그인 실행·macOS 알림 권한·에이전트 소켓은 실제 환경을
변경하지 않습니다. 메뉴와 설정 창에 `첫 실행 테스트 모드`가 표시되며
앱을 정상 종료하면 테스트용 UserDefaults가 삭제됩니다. DB는 macOS 임시
디렉터리에만 만들어져 시스템의 임시 파일 정리 대상이 됩니다.
...
- 최근 에이전트 기록 20개를 source 또는 허용된 projectPath 기준으로
  그룹핑하며 상태 아이콘·문구와 unread 표시를 제공합니다. 메뉴 막대의
  햄스터는 6번 프레임에서 쉬고, Claude 작업이 진행 중이거나 새
  Codex/Claude 기록이 미확인 상태이면 원본 GIF 프레임으로 움직입니다.
  메뉴를 열면 읽음 처리되며, 진행 중 작업이 없다면 다시 6번 프레임에서
  멈춥니다.
...
- 설정에서 Jira Cloud 사이트·이메일·Classic API token을 연결하면
  In Progress이거나 시작일·기한이 오늘 이전인 미완료 담당 이슈를 오늘
  할 일로 가져옵니다. Epic·Initiative 타입과 Hold·Backlog 상태는
  제외합니다. 토큰은 Keychain에만 저장하고 Jira에는 쓰지 않습니다.
  앱 시작·날짜 변경 시 LocalDay당 한 번만 자동 시도하며 설정의 직접
  가져오기는 별도 실행할 수 있습니다. 가져온 Jira 항목도 모래에서
  삭제할 수 있고, 같은 날 다시 동기화해도 재생성되지 않습니다. 연결된
  경우 메뉴의 오늘 할 일 제목 옆 소프트 블루 동기화 아이콘으로 즉시 한
  번 동기화할 수 있습니다.
- 앱 번들에 `hamster-event`가 포함되며 설정 화면에서 현재 절대 경로가
  반영된 Codex/Claude snippet을 복사할 수 있습니다. 사용자가 자동 설정을
  누른 경우에만 기존 파일을 백업하고 모래 항목을 병합합니다.
- 본인 전용 ad-hoc 서명 DMG 생성 스크립트와 Sprint 6 인수 체크리스트가
  제공됩니다.
- 전체 scheme의 자동화 테스트는 154개입니다 (`MoraeApp` 109개,
  `MoraeCore` 34개, `HamsterEventCLI` 11개).
...
| 마스코트(모래) | 메뉴 막대에서 에이전트 진행·미확인 상태를 애니메이션으로 표현하고, 향후 별도 오버레이 창으로 확장 | 메뉴 막대 애니메이션 구현 / 오버레이는 2단계 |
...
메뉴 막대 아이콘을 누르고 "아티클 추천받기" 버튼을 선택하면 추천 아티클을 준비합니다. MVP에서는 예약 실행이나 주기적인 백그라운드 생성을 하지 않습니다.
...
사용자가 macOS 알림 권한을 거부했거나 나중에 껐다면, 미확인 기록이
있는 동안 메뉴 막대 햄스터를 움직여 알림 없이도 눈에 띄게 하고, 설정
화면에 권한 재요청 안내를 배치합니다.
...
피드는 사용자가 메뉴 막대의 "아티클 추천받기" 버튼을 누를 때 한 번만 확인합니다. MVP에서는 예약 폴링이나 백그라운드 수집을 하지 않습니다.
...
- 공식 출처 여부
- 기본 큐레이션 소스의 품질 가중치
- AI·프론트엔드 ≫ 협업 > 인프라·데이터 > 기타 주제 우선순위

게시일이 없거나 30일보다 오래된 글은 추천 후보에서 제외합니다.
주제는 제목의 한국어·영어 키워드만으로 판별하며 원문이나 피드 본문을
분석하지 않습니다.
...
피드에서 제목, 링크, 출처와 게시일만 읽으며 AI API를 사용하지 않습니다.
피드 조회에 실패하면 아티클 영역에 오류 상태만 보여주며 자동
재시도하지 않습니다. 어제 완료와 오늘 할 일은 독립된 섹션에 계속
표시합니다.
...
- Jira Cloud 읽기 전용 자동 가져오기 (구현됨)
  ([설계](docs/JIRA_INTEGRATION_DESIGN.md))
- Apple 미리 알림 (MVP 이후 최우선 연동 대상)
- Google Calendar
- Notion
- GitHub

사용자가 연결하지 않아도 핵심 기능을 사용할 수 있습니다. Jira Cloud는
Sprint 7에서 구현했습니다. Apple 미리 알림은 Jira 다음의
온디바이스 연동 후보이며 EventKit으로 OAuth 없이 연결할 수 있어 §10
개인정보 원칙과 자연스럽게 맞습니다.
...
```toml
notify = ["/Users/<user>/Library/Application Support/Morae/bin/hamster-event"]
```
...
설정의 에이전트 탭에서 자동 설정을 실행하면 helper를
`~/Library/Application Support/Morae/bin/hamster-event`에 복사하고,
Codex `config.toml` 또는 Claude `settings.json`에 모래 항목만 병합합니다.
변경 전 원본은 같은 위치의 `*.morae-backup`에 한 번 보관합니다. Codex에
기존 `notify`가 있으면 모래 helper가 payload를 받은 뒤 기존 명령에도
전달하므로 한쪽 설정을 덮어쓰지 않습니다. 해석할 수 없는 TOML/JSON은
수정하지 않고 설정 화면의 직접 설정 스니펫으로 안내합니다.
...
- 메뉴 막대보다 큰 모래 마스코트 애니메이션과 별도 오버레이 창
- WidgetKit 위젯
- 예약 아침 브리핑과 백그라운드 수집
- 위젯 크기별 레이아웃
- 알림에서 상세 화면으로 이동
...
- Jira Cloud의 진행 중·시작일 경과·기한 초과 미완료 항목
- Git 커밋
- GitHub Pull Request
- Apple 미리 알림
- 캘린더
...
- Codex `config.toml`, Claude `settings.json` 자동 설정은 사용자가 버튼을
  누른 경우에만 수행하고, 기존 파일을 백업한 뒤 모래 항목만 병합한다.
  안전하게 해석할 수 없는 파일은 변경하지 않고 직접 설정을 제공한다.
...
- 메뉴 막대 마스코트는 MVP에 포함하고, 위젯과 별도 마스코트 오버레이는
  2단계(§2, §9)로 미룹니다.
- 아티클은 AI 요약 없이 큐레이션 피드의 제목, 링크, 출처와 게시일만
  사용하고, 최근 30일의 개별 글 중심 소스와 품질 가중치를 반영합니다
  (§4.1, [ADR-0014](docs/adr/0014-individual-article-recommendations.md)).
- 제목 기반 주제 우선순위는 AI·프론트엔드 ≫ 협업 >
  인프라·데이터 > 기타로 적용합니다
  ([ADR-0015](docs/adr/0015-metadata-topic-priority.md)).
...
2. 메뉴 막대에서 브리핑 생성을 요청하면 오늘의 아티클 섹션에서 최근
   아티클 한 개의 제목·출처·게시일·원문 링크를 확인할 수 있고, 어제
   완료와 오늘 할 일은 각각 독립된 섹션에서 확인할 수 있다.
3. 오늘 할 일을 추가·수정·완료·삭제할 수 있고, 앱이 계속 실행 중이어도
   시스템 날짜가 바뀌면 완료한 항목이 “어제 한 일”에 나타난다.
